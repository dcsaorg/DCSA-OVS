...
CREATE TABLE requested_equipment (
    ...
    requested_equipment_sizetype varchar(4) NOT NULL REFERENCES iso_equipment_code (iso_equipment_code),
    requested_equipment_units integer NOT NULL,
    confirmed_equipment_sizetype varchar(4)  NULL REFERENCES iso_equipment_code (iso_equipment_code),
    confirmed_equipment_units integer NULL,
    is_shipper_owned boolean NOT NULL DEFAULT false
);

CREATE INDEX ON requested_equipment (booking_id);
...
CREATE TABLE displayed_address (
    document_party_id uuid NOT NULL REFERENCES document_party (id),
    address_line_number integer NOT NULL,
    address_line_text varchar(250) NOT NULL
);
CREATE INDEX ON displayed_address (document_party_id, address_line_number);
...
CREATE TABLE equipment (
    ...
    iso_equipment_code char(4) NULL REFERENCES iso_equipment_code (iso_equipment_code),
    tare_weight real NULL,
    weight_unit varchar(3) NULL REFERENCES unit_of_measure(unit_of_measure_code)  CHECK (weight_unit IN ('KGM','LBR'))
);
...
CREATE TABLE requested_equipment_equipment (
    requested_equipment_id uuid NOT NULL REFERENCES requested_equipment(id),
...
CREATE TABLE active_reefer_settings (
    utilized_transport_equipment_id uuid PRIMARY KEY REFERENCES utilized_transport_equipment (id),
...
CREATE INDEX ON consignment_item (hs_code);
...
CREATE TABLE seal (
    ...
    seal_type_code varchar(5) NOT NULL REFERENCES seal_type (seal_type_code)
);
-- Supporting FK constraints
