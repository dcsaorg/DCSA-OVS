...
CREATE TABLE event_subscription_transport_document_type (
    subscription_id uuid NOT NULL REFERENCES event_subscription (subscription_id) ON DELETE CASCADE,
    transport_document_type_code varchar(4) NOT NULL REFERENCES transport_document_type (transport_document_type_code),
    PRIMARY KEY (subscription_id, transport_document_type_code)
);

CREATE TABLE event_subscription_shipment_event_type (
    subscription_id uuid NOT NULL REFERENCES event_subscription (subscription_id) ON DELETE CASCADE,
...
CREATE TABLE event_subscription_transport_event_type (
    subscription_id uuid NOT NULL REFERENCES event_subscription (subscription_id) ON DELETE CASCADE,
...
CREATE TABLE event_subscription_equipment_event_type (
    subscription_id uuid NOT NULL REFERENCES event_subscription (subscription_id) ON DELETE CASCADE,
...
CREATE TABLE event_subscription_operations_event_type (
    subscription_id uuid NOT NULL REFERENCES event_subscription (subscription_id) ON DELETE CASCADE,
...
CREATE TABLE unmapped_event_queue (
...
-- Only used by UI support to assist the UI
CREATE OR REPLACE VIEW ui_timestamp_info AS
    SELECT operations_event.event_id,
       ...
       operations_event.transport_call_id
        FROM operations_event
                 LEFT JOIN ops_event_timestamp_definition ON (ops_event_timestamp_definition.event_id = operations_event.event_id)
